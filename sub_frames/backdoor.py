...
def print_main_options():
    options = {
        'generator': 'Use this option to generate backdoors',
        'listener': 'Use this options to start the listenr against the victim'
    }
...
def main():
    ...
    while True:
        ...
        if command == 'use generator':
            activated_option.append(' {}generate({}Backdoor{}{}){}'.format(colors.BOLD, colors.Red, colors.RESET, colors.BOLD, colors.RESET))
            execfile_y(f'{program_path}/libs/backdoor/generator.py', global_dict)

        # if the command is to activate the listenet
        elif command == 'use listener':
            activated_option.append(' {}listen({}Backdoor{}{}){}'.format(colors.BOLD, colors.Red, colors.RESET, colors.BOLD, colors.RESET))
            execfile_y(f'{program_path}/libs/backdoor/listener.py', global_dict)
        ...
        # if the command is exit or quit breake the progrm
            ...
            activated_option.remove('/backdoor')
            break
